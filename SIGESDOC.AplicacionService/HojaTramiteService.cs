...
namespace SIGESDOC.AplicacionService
{
    public class HojaTramiteService : IHojaTramiteService
    {
        ...
        private readonly IDetalleDocumentoExternoRepositorio _DetalleDocumentoExternoRepositorio;
        private readonly IDocumentoInternoExternoDhcpaRepositorio _DocumentoInternoExternoDhcpaRepositorio;

        private readonly IMaeDocumentoDhcpaExternoRepositorio _MaeDocumentoDhcpaExternoRepositorio;
        private readonly IMaeDocumentoDhcpaDetalleExternoRepositorio _MaeDocumentoDhcpaDetalleExternoRepositorio;
        private readonly IColaboradoresRefirmaRepositorio _ColaboradoresRefirmaRepositorio;
        private readonly IDatDocumentodetalleDocinternosRepositorio _DatDocumentodetalleDocinternosRepositorio;
        ...
        IDetalleMaeDocumentoRepositorio DetalleMaeDocumentoRepositorio,
            IDetalleDocumentoExternoRepositorio DetalleDocumentoExternoRepositorio,
            IDocumentoInternoExternoDhcpaRepositorio DocumentoInternoExternoDhcpaRepositorio,
            IMaeDocumentoDhcpaExternoRepositorio MaeDocumentoDhcpaExternoRepositorio,
            IMaeDocumentoDhcpaDetalleExternoRepositorio MaeDocumentoDhcpaDetalleExternoRepositorio,
            IColaboradoresRefirmaRepositorio ColaboradoresRefirmaRepositorio,
            IDatDocumentodetalleDocinternosRepositorio DatDocumentodetalleDocinternosRepositorio
            ...
            )
        {
            ...
            _DetalleDocumentoExternoRepositorio = DetalleDocumentoExternoRepositorio;
            _DocumentoInternoExternoDhcpaRepositorio = DocumentoInternoExternoDhcpaRepositorio;

            _MaeDocumentoDhcpaExternoRepositorio = MaeDocumentoDhcpaExternoRepositorio;
            _MaeDocumentoDhcpaDetalleExternoRepositorio = MaeDocumentoDhcpaDetalleExternoRepositorio;
            _ColaboradoresRefirmaRepositorio = ColaboradoresRefirmaRepositorio;
            _DatDocumentodetalleDocinternosRepositorio = DatDocumentodetalleDocinternosRepositorio;
    }
        ...
        public IEnumerable<DetalleDocumentoExternoResponse> Listar_Detalle_Documento(int id_documento)
        {
            var result = (from x in _DetalleDocumentoExternoRepositorio.Listar()
                          where x.ID_DOC_DHCPA == id_documento
                          select new DetalleDocumentoExternoResponse
                          { 
                          
                              id_doc_dhcpa = x.ID_DOC_DHCPA,
                              id_oficina_direccion_destino = x.ID_OFICINA_DIRECCION_DESTINO,
                              persona_destino = x.PERSONA_DESTINO, 
                              activo = x.ACTIVO, 
                              nombre_persona_destino = x.NOMBRE_PERSONA_DESTINO, 
                              flag_tipo_carta = x.FLAG_TIPO_CARTA, 
                              encargado = x.ENCARGADO, 
                              cargo = x.CARGO, 
                              oficina_externa = x.OFICINA_EXTERNA, 
                              oficina_sede = x.OFICINA_SEDE, 
                              direccion_externo = x.DIRECCION_EXTERNO
                          
                          });
            return result.ToList();
        }

        public IEnumerable<DocumentoInternoExternoDhcpaResponse> Listar_Detalle_Documento_Interno_Externo(int num_Doc)
        {

            var result = (from x in _DocumentoInternoExternoDhcpaRepositorio.Listar()
                          where x.ID_TIPO_DOCUMENTO == num_Doc
                          select new DocumentoInternoExternoDhcpaResponse
                          {
                              num_doc = x.NUM_DOC,
                              id_tipo_documento = x.ID_TIPO_DOCUMENTO, 
                              nom_doc = x.NOM_DOC, 
                              fecha_doc = x.FECHA_DOC, 
                              asunto = x.ASUNTO, 
                              anexos = x.ANEXOS,
                              fecha_registro = x.FECHA_REGISTRO, 
                              id_filial = x.ID_FILIAL,
                              numero_ht = x.NUMERO_HT, 
                              id_oficina_direccion = x.ID_OFICINA_DIRECCION, 
                              evaluador_cdl_notif = x.EVALUADOR_CDL_NOTIF, 
                              direccion_cdl_notif = x.DIRECCION_CDL_NOTIF,
                              empresa_cdl_notif = x.EMPRESA_CDL_NOTIF,
                              folia_cdl_notif = x.FOLIA_CDL_NOTIF, 
                              doc_notificar_cdl_notif = x.DOC_NOTIFICAR_CDL_NOTIF, 
                              exp_o_ht_cdl_notif = x.EXP_O_HT_CDL_NOTIF,
                              exp_o_ht_n_cdl_notif = x.EXP_O_HT_N_CDL_NOTIF, 
                              ruc = x.RUC, 
                              id_det_documento = x.ID_DET_DOCUMENTO
                          });
            return result.ToList();
        }

        public IEnumerable<MaeDocumentoDhcpaExternoResponse> Listar_Documento_DHCPA_Externo(int id_documento)
        {

            var result = (from x in _MaeDocumentoDhcpaExternoRepositorio.Listar()
                          where x.ID_DOC_DHCPA == id_documento
                          select new MaeDocumentoDhcpaExternoResponse { 
                          
                              id_doc_dhcpa = x.ID_DOC_DHCPA,
                              nom_doc = x.NUM_DOC + x.NOM_DOC,
                              id_tipo_documento = x.ID_TIPO_DOCUMENTO,
                              asunto = x.ASUNTO, 
                              num_doc = x.NUM_DOC,
                          });
        ...
        public IEnumerable<MaeDocumentoDhcpaDetalleExternoResponse> Listar_Documento_DHCPA_Detalle_Externo(int id_documento)
        {
            var result = (from x in _MaeDocumentoDhcpaDetalleExternoRepositorio.Listar()
                          where x.ID_DOC_DHCPA == id_documento
                          select new MaeDocumentoDhcpaDetalleExternoResponse
                          {
                              id_doc_dhcpa_det = x.ID_DOC_DHCPA_DET,
                              encargado = x.ENCARGADO,
                              cargo = x.CARGO,
                              direccion_externo = x.DIRECCION_EXTERNO,
                              oficina_externa = x.OFICINA_EXTERNA,
        ...
        public IEnumerable<ColaboradoresRefirmaResponse> GetColaboradores(string nombres)
        {
            var result = (from x in _ColaboradoresRefirmaRepositorio.Listar(xp => xp.NOMBRES.ToUpper().StartsWith(nombres))
                         // where x.NOMBRES.ToUpper().StartsWith(nombres.Trim())
                          select new ColaboradoresRefirmaResponse
                          { 
                              persona_num_documento = x.persona_num_documento,
                              nombres = x.NOMBRES
                          });
            return result.ToList();
        }

        public IEnumerable<DatDocumentodetalleDocinternosResponse> GetDocumentoDetalleInterno(int id_documento)
        {
            var result = (from x in _DatDocumentodetalleDocinternosRepositorio.Listar(i => i.ID_DOCUMENTO == id_documento)
                          select new DatDocumentodetalleDocinternosResponse
                          {
                              id_documento = x.ID_DOCUMENTO,
                              nom_oficina_crea = x.NOM_OFICINA_CREA,
                              nom_oficina_destino = x.NOM_OFICINA_DESTINO
