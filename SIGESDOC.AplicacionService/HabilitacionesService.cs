...
namespace SIGESDOC.AplicacionService
{
    public class HabilitacionesService : IHabilitacionesService
    {
        ...
        private readonly IFirmaDigitalRepositorio _FirmaDigitalRepositorio; 
        ...
        public HabilitacionesService(
            ...
            IUidAlfrescoRepositorio UidAlfrescoRepositorio,
            IFirmaDigitalRepositorio FirmaDigitalRepositorio
            ...
            )
        {
            ...
            _uidAlfrescoRepositorio = UidAlfrescoRepositorio;
            //_uidAlfrescoRepositorio = BuscarNuevoUIIDALfrescoRepositorio;
            //_uidAlfrescoRepositorio = ActualizarUIIDALFRESCORepositorio;
            _FirmaDigitalRepositorio = FirmaDigitalRepositorio;
        }
        ...
        public void Create_Firma_Digital(int id_documento, string persona_firmar, int id_usuario_envio, bool estado_envio, DateTime fecha_firma, int id_uid_alfresco, bool estado_persona_firmar)
        {
            try
            {
                using(TransactionScope scope = new TransactionScope())
                {
                    _FirmaDigitalRepositorio.Create_Firma_Digital(id_documento, persona_firmar, id_usuario_envio, estado_envio, fecha_firma, id_uid_alfresco, estado_persona_firmar);
                    _unitOfWork.Guardar();
                    scope.Complete();
                }
            }
      ...
      public void Update_Firma_Digital_Activar_Visado(int id_documento, bool estado_persona_firmar, string persona_firmar)
      {
            try
            {
                using(TransactionScope scope = new TransactionScope())
                {
                    _FirmaDigitalRepositorio.Update_Firma_Digital_Activar_Visado(id_documento, estado_persona_firmar, persona_firmar);
                    _unitOfWork.Guardar();
                    scope.Complete();
                }
            }
      ...
      public IEnumerable<FirmaDigitalResponse> Listar_FirmaDigital(int id)
      {
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    return _FirmaDigitalRepositorio.Listar_FirmaDigital(id);
                }
            }
            catch
            {
        ...
        public void InserirNuevoUIIDALfresco_FirmaDigital(string tipodocumento, string nom_doc, string num_doc, string uidAlfresco, string byteDoc)
        {
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    ...
                    _uidAlfrescoRepositorio.InserirNuevoUIIDALfresco(nom_doc, tipodocumento, uidAlfresco, 1, date, "FIRMADO", null, null, null, byteDoc, 0);
                    _unitOfWork.Guardar();
                    scope.Complete();
                }
        ...
        public void InserirNuevoUIIDALfresco_SinFirmaDigital(string tipodocumento, string nom_doc, string num_doc, string uidAlfresco, string byteDoc)
        {
            try
            {
                using (TransactionScope scope = new TransactionScope())
                {
                    var date = DateTime.Now;
                    _uidAlfrescoRepositorio.InserirNuevoUIIDALfresco(nom_doc, tipodocumento, uidAlfresco, 1, date, "AGUARDANDO FIRMAR", null, null, null, byteDoc, 0);
                    _unitOfWork.Guardar();
                    scope.Complete();
                }
